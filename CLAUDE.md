...
humanity — without any discrimination based on religion, language, race, gender,
...
- Zero discrimination — enforced at code level
...
├── .vibe/                       ← Vibe-coding platform config
│   ├── config.yml               ← Model routing & AI provider config
│   ├── agents/                  ← Per-agent CLAUDE.md overrides
│   │   ├── developer.md
│   │   ├── analyst.md
│   │   ├── tester.md
│   │   ├── reviewer.md
...
│       ├── plan.md
│       └── assign.md
├── backlog/                     ← Backlog.md task store
│   ├── epics/
│   ├── features/
│   └── tasks/
├── concept/                     ← Product concept docs (read-only reference)
│   ├── MHS_Custom_Instructions.md
│   ├── MHS_KB_01_Vision_Market.md
│   ├── MHS_KB_02_Technical.md
│   └── MHS_KB_03_UX_Business_Ethics.md
├── apps/
...
# These must NEVER appear in scoring functions
```

---

## 🎯 MHS Score Architecture (Quick Reference)
...
- Consistency multiplier (up to 1.3×)
- Geographic equity multiplier (up to 1.3×)
...
1. **NEVER** add discrimination-based logic to the scoring system
2. **ALWAYS** read the relevant backlog task before starting work
3. **ALWAYS** write tests alongside implementation (TDD preferred)
4. **NEVER** hardcode API keys or secrets — use environment variables
5. **ALWAYS** update task status in backlog after completing work
6. **NEVER** break existing tests without explicit approval
7. **ALWAYS** follow the ADR process for architecture decisions
8. Read `concept/` docs before implementing any scoring logic
...
# AI Providers (for Angel AI & features)
ANTHROPIC_API_KEY=...
OPENAI_API_KEY=...
GOOGLE_AI_API_KEY=...
...
*This file is read at the start of every agent session. Keep it under 200 lines.*
