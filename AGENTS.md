# AGENTS.md — Multi-AI Agent Routing & Role Definitions

## Overview

This project uses a **model-agnostic multi-agent system**. Any AI tool
(Claude Code, Cursor, GitHub Copilot, Gemini CLI, Codex) can be used.
The `.vibe/config.yml` defines which model handles which task type,
and you can override per-task or per-session.
...
### 🔬 Researcher
**Purpose:** Deep research, competitive analysis, requirement gathering
**Default model:** `gemini-2.5-pro` (large context, web search)
**Fallback:** `claude-opus-4-6`
**Typical tasks:**
- Analyze similar platforms (Klout, Goodwall, etc.)
- Research scoring methodologies
- Gather API documentation for integrations
- Analyze academic papers on impact measurement

**Instructions file:** `.vibe/agents/researcher.md`

---

### 🏗️ Analyst
**Purpose:** Architecture decisions, PRD creation, task decomposition
**Default model:** `claude-opus-4-6` (complex reasoning)
**Fallback:** `o3` (OpenAI)
**Typical tasks:**
- Create/refine Epics and Features
- Break features into implementation tasks
- Architecture Decision Records (ADRs)
- Database schema design
- API contract definition

**Instructions file:** `.vibe/agents/analyst.md`

---

### 💻 Developer
**Purpose:** Code implementation
**Default model:** `claude-sonnet-4-6` (cost/quality balance)
**Fallback:** `gpt-4o` or `gemini-2.5-flash`
**Typical tasks:**
...
**Instructions file:** `.vibe/agents/developer.md`

---

### 🧪 Tester
**Purpose:** QA, test writing, test execution
**Default model:** `claude-haiku-4-5` (fast, cheap for repetitive work)
...
**Instructions file:** `.vibe/agents/tester.md`
...
- Check ethics compliance (no discrimination logic)
- Security review
- Performance analysis
- Approve or request changes

**Instructions file:** `.vibe/agents/reviewer.md`

---

### 📊 Data Analyst

**Purpose:** Master data management, data quality, data labeling
**Default model:** `claude-opus-4-6` (complex data reasoning)
**Fallback:** `o3`

**Typical tasks:**

- Run data quality checks (completeness, accuracy, consistency, uniqueness)
- Build and maintain labeled training datasets for ML models
- Define canonical entity definitions and controlled vocabularies
- Produce weekly data health reports
- Write Great Expectations data contracts

**Instructions file:** `.vibe/agents/data-analyst.md`

---

### 🕷️ Data Crawler

**Purpose:** Collect and integrate data from external sources
**Default model:** `gemini-2.5-pro` (web search + 1M context)
**Fallback:** `claude-sonnet-4-6`

**Typical tasks:**

- Build OAuth 2.0 integrations (GitHub, LinkedIn, ORCID)
- Write Scrapy/httpx crawlers for NGO databases and public registries
- Maintain Celery Beat schedules for recurring data syncs
- Implement crawl state persistence and resumable crawls
- Ensure robots.txt compliance and rate limiting

**Instructions file:** `.vibe/agents/data-crawler.md`

---

### 📈 Data Visualizer

**Purpose:** Build charts, dashboards, and data widgets
**Default model:** `claude-sonnet-4-6` (strong HTML/CSS/React)
**Fallback:** `gpt-4o`

**Typical tasks:**

- Build Recharts/D3 components (radar, bar, line, choropleth)
- Design and implement admin and public statistics dashboards
- Create anonymized public stats views
- Build bias audit visual reports
- Ensure accessibility (axe-core clean, color-blind safe)

**Instructions file:** `.vibe/agents/data-visualizer.md`
...
| `implement:` | Developer | Code writing |
| `fix:` | Developer | Bug fix |
| `test:` | Tester | QA work |
| `review:` | Reviewer | Code review |
| `prd:` | Analyst | Product spec |
| `migrate:` | Developer | DB migration |
| `deploy:` | Developer | Infrastructure |
| `quality:` | Data Analyst | Data quality checks |
| `label:` | Data Analyst | Dataset labeling |
| `mdm:` | Data Analyst | Master data management |
| `crawl:` | Data Crawler | Web/API data collection |
| `integrate:` | Data Crawler | Platform OAuth integration |
| `sync:` | Data Crawler | Scheduled data sync |
| `visualize:` | Data Visualizer | Charts and dashboards |
| `dashboard:` | Data Visualizer | Analytics UI |
| `chart:` | Data Visualizer | Data visualization component |
...
|----------|-------------|-----|
| Complex architecture decisions | `claude-opus-4-6` / `o3` | Best reasoning |
| Everyday coding | `claude-sonnet-4-6` | Best cost/quality |
| Fast repetitive tasks (tests, boilerplate) | `claude-haiku-4-5` / `gpt-4o-mini` / `gemini-2.5-flash` | Cheapest |
| Large codebase analysis | `gemini-2.5-pro` | 1M token context |
| Research + web search | `gemini-2.5-pro` | Native search |
| Math/algorithm heavy | `o3` / `o4-mini` | Best reasoning |
| Frontend/UI work | `claude-sonnet-4-6` | Strong HTML/CSS/React |

---

## 💰 Cost Optimization Rules

1. **Use the cheapest model that gets the job done**
2. Haiku/Flash for test writing, boilerplate, migrations
3. Sonnet for main development
4. Opus/o3 only for architecture or complex debugging
5. Cap parallel agents at 3 to avoid runaway costs
6. Add `.claudeignore` to skip `node_modules`, `dist`, `.next`
7. Keep `CLAUDE.md` under 200 lines (injected every request)
8. Use `--continue` flag to resume sessions (avoids re-reading context)

---

## 🔌 AI Tool Compatibility

This project is designed to work with any of:

| Tool | How to use | Best for |
|------|-----------|---------|
| **Claude Code** | `claude` in terminal | Main development |
| **Cursor** | Open repo in Cursor | Frontend work, IDE-heavy |
| **GitHub Copilot** | VS Code extension | Inline completion |
| **Gemini CLI** | `gemini` in terminal | Research, large context |
| **OpenAI Codex** | API or Codex CLI | Algorithm-heavy tasks |
| **Aider** | `aider` in terminal | Multi-file changes |

All tools read `CLAUDE.md` / `AGENTS.md` at startup.
Backlog tasks are plain Markdown — any tool can read/update them.

---

## 📋 Workflow: How to Start a Task

### Option A: Single Agent
```bash
# 1. Pick a task
backlog task view task-001

# 2. Start Claude Code with the task
claude "Read backlog/tasks/task-001-user-profile-api.md and implement it"

# 3. When done, mark complete
backlog task done task-001
```

### Option B: Multi-Agent (Claude Code Agent Teams)
```bash
# Enable agent teams (add to settings.json)
# "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"

claude "Create an agent team: 
  - developer (claude-sonnet-4-6): implement task-001
  - tester (claude-haiku-4-5): write tests for task-001
  - reviewer: review the output"
```

### Option C: Mixed AI Tools
```bash
# Research phase with Gemini
gemini "Read concept/MHS_KB_01_Vision_Market.md and analyze 
        competitors for the badge system feature"

# Implementation with Claude Code
claude "Read .vibe/agents/developer.md and implement 
        backlog/tasks/task-015-badge-system.md"

# Tests with Codex or Claude Haiku
# (create subtask for tester agent)
```

---

*Keep this file updated when adding new agent roles or model options.*
